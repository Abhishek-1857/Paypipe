...
function SuccessState({ wallet, companyName }: { name: string; wallet: string; companyName: string | null }) {
  ...
  return (
    <div style={{ textAlign: "center", padding: "32px 0" }}>
      {/* Animated checkmark */}
      <div style={{ width: 72, height: 72, borderRadius: "50%", background: "rgba(0,217,126,0.1)", border: "2px solid rgba(0,217,126,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px", animation: "scaleIn 0.4s ease" }}>
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#00D97E" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ animation: "drawCheck 0.4s ease 0.2s both" }}>
      ...
      <div style={{ background: "#161618", border: "1px solid #2A2A30", borderRadius: 8, padding: "12px 16px", marginBottom: 20 }}>
        <p style={{ margin: "0 0 4px", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#5A5A6A" }}>Your registered wallet</p>
        <p style={{ margin: 0, fontFamily: "monospace", fontSize: 14, color: "#00D97E", fontWeight: 600 }}>{truncated}</p>
      </div>

      {/* Info box */}
      <div style={{ background: "rgba(0,217,126,0.05)", border: "1px solid rgba(0,217,126,0.15)", borderRadius: 8, padding: "12px 16px", textAlign: "left" }}>
        <p style={{ margin: 0, fontSize: 13, color: "#8A8A96", lineHeight: 1.6 }}>
          <span style={{ color: "#00D97E" }}>ℹ</span> You&apos;ll receive an email confirmation when a payment is sent to you.
        </p>
      </div>
...
export default function InvitePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0F19", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
          ...
          {pageState === "form" && (
            <>
              <h1 style={{ margin: "0 0 8px", fontSize: 22, fontWeight: 800, color: "#F2F2F3", lineHeight: 1.3 }}>
                {companyName
                  ? <>You&apos;ve been invited to join <span style={{ color: "#00D97E" }}>{companyName}</span>&apos;s contractor network</>
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                {/* Name */}
                <div>
                  <label style={{ display: "block", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#5A5A6A", marginBottom: 6, fontWeight: 600 }}>
                    Full Name *
                  </label>
                  <input
                    required
                ...
                <div>
                  ...
                  />
                  <p style={{ margin: "8px 0 0", fontSize: 12, color: "#5A5A6A", lineHeight: 1.5 }}>
                    Don&apos;t have one?{" "}
                    <a href="https://phantom.app" target="_blank" rel="noopener noreferrer" style={{ color: "#00D97E", textDecoration: "none" }}>
                ...
                {/* Info box */}
                <div style={{ background: "rgba(0,217,126,0.05)", border: "1px solid rgba(0,217,126,0.12)", borderRadius: 8, padding: "12px 14px" }}>
                  <p style={{ margin: 0, fontSize: 12, color: "#8A8A96", lineHeight: 1.6 }}>
                    <span style={{ color: "#00D97E", fontWeight: 700 }}>What is a Solana wallet?</span>{" "}
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  style={{ width: "100%", padding: "13px", background: submitting ? "#0D2E1F" : "#00D97E", color: submitting ? "#00D97E" : "#0A0A0B", border: "none", borderRadius: 8, fontSize: 15, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer", transition: "background 0.2s", marginTop: 4 }}
                >
